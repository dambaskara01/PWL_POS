...
class StokController extends Controller
{
    ...
    public function create_ajax()
    {
        $barang = BarangModel::select('barang_id', 'barang_nama')->get();
        $user = UserModel::select('user_id', 'username')->get();
        $supplier = SupplierModel::select('supplier_id', 'nama_supplier')->get(); // Ambil data supplier
    ...
    public function edit_ajax(string $id)
    {
        ...
        $supplier = SupplierModel::select('supplier_id', 'nama_supplier')->get(); // Ambil daftar supplier
    ...
    public function delete_ajax(Request $request, $id)
    {
        if ($request->ajax() || $request->wantsJson()) {
            ...
            if ($stok) {
                DB::beginTransaction();
                try {
                    // Simpan jumlah stok sebelum dihapus
                    $jumlahStok = $stok->stok_jumlah;
                    $barangId   = $stok->barang_id;

                    // Hapus data stok
                    $stok->delete();

                    // Kurangi stok barang sesuai jumlah stok yang dihapus
                    BarangModel::where('barang_id', $barangId)
                        ->decrement('barang_stok', $jumlahStok);
    ...
    public function export_excel()
    {
        ...
        foreach ($stok as $key => $value) {
            ...
            $sheet->setCellValue('D' . $baris, $value->supplier->nama_supplier);
